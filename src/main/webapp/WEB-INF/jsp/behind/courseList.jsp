...
<html>
	<head>
		...
		function delCourseById(Obj,id,courseTitle){
			//alert(id);
		 Confirm.show('温馨提示：', '确定要删除'+courseTitle+'么？', {
                'Delete': {
                    'primary': true,
                    'callback': function() {
                    	var param={"id":id};
      ...
      <tbody>
        <c:forEach items="${pageInfo.list}" var="course" varStatus="status">
            <tr>
	          ...
	          <td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;" >${course.courseTitle}</td>
	          
	          <td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${course.courseDesc}</td>
	          <td>${course.subjectName}</td>
	          <td><a href="${pageContext.request.contextPath}/course/edit/${course.id}"><span class="glyphicon glyphicon glyphicon-edit" aria-hidden="true"></span></a></td>
		...
		<div class="navbar-right">
			<nav aria-label="Page navigation">
				<ul class="pagination">
					...
					<li>
						<a  href="${pageContext.request.contextPath}/course/list?page=1&pageSize=${pageInfo.pageSize}" aria-label="Previous">
					...
					<li>
						<a href="${pageContext.request.contextPath}/course/list?page=${pageInfo.pages}&pageSize=${pageInfo.pageSize}" aria-label="Previous">
...
<script>
	...
	function toProPage() {
		...
		//alert(prePage);
		location.href = "${pageContext.request.contextPath}/course/list?page="+prePage+"&pageSize=${pageInfo.pageSize}"
	}
	...
	function toNextPage() {
		...
		location.href = "${pageContext.request.contextPath}/course/list?page="+netPage+"&pageSize=${pageInfo.pageSize}"
}
